function [A, sortedjd, distance, valid_indices] = Get_A1_trangle_simple_full(station, satellite, jdmin, jdmax, jdjg, wdmin, wdmax, wdjg, gdmin, gdmax, gdjg,jdmin_rad,wdmin_rad,jdmax_rad,wdmax_rad,jdjg_rad,wdjg_rad,total_length,jd_grid,wd_grid,h_grid)
% Simplified version - Skips regional checks, directly calculates all rays.

% --- Calculate Grid Dimensions ---
nj = round((jdmax - jdmin) / jdjg); % Number of longitude cells
nw = round((wdmax - wdmin) / wdjg); % Number of latitude cells
ng = round((gdmax - gdmin) / gdjg); % Number of height cells
...
for i = 1:n_rays
    ...
    try
        [A1, sorted_ddzb, distances] = Get_sta_A1_tradition_optimized_full(...
