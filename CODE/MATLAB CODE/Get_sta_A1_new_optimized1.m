...
function [one_dimensional_index,sortedjd_ddzb,values] =Get_sta_A1_new_optimized1(XD,YD,ZD,XS,YS,ZS,jdmin,jdmax,jdjg,wdmin,wdmax,wdjg,gdmin,gdmax,gdjg,jdmin_rad,wdmin_rad,jdmax_rad,wdmax_rad,jdjg_rad,wdjg_rad,total_length,nj,nw,ng,h_grid)
% 初始化输出
...
% 把要合并的三个向量显式做成列向量并一次拼成 n×3 矩阵（避免先构造 3×n 再转置）
...
% 1. 预先计算中心点和半宽（如果在循环外计算，效率更高）
% wd_mid = (wdmin_rad + wdmax_rad) * 0.5;
% wd_rng = (wdmax_rad - wdmin_rad) * 0.5;
% 
% jd_mid = (jdmin_rad + jdmax_rad) * 0.5;
% jd_rng = (jdmax_rad - jdmin_rad) * 0.5;
% 
% gd_mid = (gdmin + gdmax) * 0.5;
% gd_rng = (gdmax - gdmin) * 0.5;
% 
% % 2. 使用绝对值判断，将6次比较减少为3次
% % 注意：MATLAB是列主序存储，直接访问列(:,1)是连续内存，速度很快
% valid_mask = (abs(all_points(:,1) - wd_mid) <= wd_rng) & ...
%              (abs(all_points(:,2) - jd_mid) <= jd_rng) & ...
%              (abs(all_points(:,3) - gd_mid) <= gd_rng);
% 
...
% midpoint_blh(2,:) = round(midpoint_blh(2,:),13);
% midpoint_blh(1,:) = round(midpoint_blh(1,:),13);
