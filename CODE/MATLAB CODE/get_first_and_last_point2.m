function [lon1, lon2, lat1, lat2, is_valid, B_U, L_U, H_U, A, z, R] = get_first_and_last_point2(jdmin_rad, jdmax_rad, wdmin_rad, wdmax_rad, gdmin, gdmax, XD, YD, ZD, XS, YS, ZS, jdjg_rad,wdjg_rad)
    ...
    lon1 = max(approximateNumberDown(min_lon, jdjg_rad)- 2*jdjg_rad , jdmin_rad);
    lon2 = min(approximateNumberUp(max_lon, jdjg_rad)+ 2*jdjg_rad , jdmax_rad);
    lat1 = max(approximateNumberDown(min_lat, wdjg_rad)- 2*wdjg_rad , wdmin_rad);
    lat2 = min(approximateNumberUp(max_lat, wdjg_rad)+ 2*wdjg_rad , wdmax_rad);    
    is_valid = true;
end