function [A, sortedjd, distance] = Get_A1_initial_simple1(station, satellite, jdmin, jdmax, jdjg, wdmin, wdmax, wdjg, gdmin, gdmax, gdjg,total_length)
...
parfor i = 1:n_rays
    ...
    % Directly calculate the projection matrix - add error handling
    try
        [A1, sorted_ddzb, distances] = Get_sta_A1_initial(...
...
%     sortedjd = struct();
%     distance = struct();
%     fprintf('Warning: No valid ray data\n');
%     return;
% end

% --- Pre-compute total number of non-zero elements ---
total_non_zeros = sum(double(non_zero_counts(valid_mask)));
total_rows = n_valid;

% --- Pre-allocate arrays - use more efficient data types ---
