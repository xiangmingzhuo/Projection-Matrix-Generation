...
lon1 = max(approximateNumberDown(min(enter_point(:,1),exit_point(:,1)),jdjg)-2*jdjg,jdmin);
lon2 = min(approximateNumberUp(max(enter_point(:,1),exit_point(:,1)),jdjg)+2*jdjg,jdmax);
lat1 = max(approximateNumberDown(min(enter_point(:,2),exit_point(:,2)),wdjg)-2*wdjg,wdmin);
lat2 = min(approximateNumberUp(max(enter_point(:,2),exit_point(:,2)),wdjg)+2*wdjg,wdmax);
h1 = gdmin;
h2 = gdmax;
end