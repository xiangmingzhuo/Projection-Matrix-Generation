...
% 创建图形窗口
f1 = figure('Position', [100, 100, 800, 600]); % 设置图形大小

% 绘制散点图
h1 = scatter(result(:,1), result(:,3), 80, 'filled', 'MarkerFaceColor', [0.2, 0.6, 0.8], 'MarkerEdgeColor', 'none');
hold on;
h2 = scatter(result(:,1), result(:,4), 80, 'filled', 'MarkerFaceColor', [0.8, 0.4, 0.2], 'MarkerEdgeColor', 'none');
...
% 如果数据点较多，添加趋势线
if size(result,1) > 2
    p1 = polyfit(result(:,1), result(:,3), 1);
    p2 = polyfit(result(:,1), result(:,4), 1);
...
hold off;
exportgraphics(f1,'figure1.png','Resolution',600);