% This function uses the parametric equation method to generate the A-matrix for a single ray
function [one_dimensional_index, sortedjd_ddzb, distances] = Get_sta_A1_canshu(XD,YD,ZD,XS,YS,ZS,jdmin,jdmax,jdjg,wdmin,wdmax,wdjg,gdmin,gdmax,gdjg,total_length,nj,nw,ng)
% Initialize output
...
orientVector = [XS,YS,ZS] - [XD,YD,ZD]; % Calculate direction vector
...
valid_quad = abs(A_term) > tol & disc >= -tol;
disc(valid_quad) = max(disc(valid_quad), 0);

t1 = nan(size(lat_grid));
t2 = nan(size(lat_grid));

t1(valid_quad) = (-B_term(valid_quad) - sqrt(disc(valid_quad))) ./ (2*A_term(valid_quad));
t2(valid_quad) = (-B_term(valid_quad) + sqrt(disc(valid_quad))) ./ (2*A_term(valid_quad));

valid_linear = abs(A_term) <= tol & abs(B_term) > tol;
t1(valid_linear) = -C_term(valid_linear) ./ B_term(valid_linear);

t_lat = [t1(:); t2(:)];
t_lat = t_lat(isfinite(t_lat));
...
% Batch calculate intersection coordinates
...
% sortedjd_ddzb(1:2,:) = round(sortedjd_ddzb(1:2,:),5);
...
% midpoint_blh(:,1:2) = round(midpoint_blh(:,1:2),5);
