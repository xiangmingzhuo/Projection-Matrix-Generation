function [A, sortedjd, distance] = Get_A1_canshu_simple2(station, satellite, jdmin, jdmax, jdjg, wdmin, wdmax, wdjg, gdmin, gdmax, gdjg,total_length)
% Simplified version - Skips region checks, directly processes all rays.
...
% Using a single parfor loop to avoid complex slicing variable issues.
...
for i = 1:n_rays
    ...
    % Calculate the projection matrix directly - with error handling
    try
        [A1, sorted_ddzb, distances] = Get_sta_A1_canshu_optimized(...
            sx, sy, sz, sat_x, sat_y, sat_z, ...
            jdmin, jdmax, jdjg, wdmin, wdmax, wdjg, gdmin, gdmax, gdjg,total_length,nj,nw,ng);
        
        if isempty(A1)
            % Ray does not intersect the model grid
            all_rows{i} = [];
            all_cols{i} = [];
            all_vals{i} = [];
            non_zero_counts(i) = 0;
...
% Identify valid rays (those that intersected the grid)
...
% Precompute the total number of non-zero elements for efficient preallocation
total_non_zeros = sum(double(non_zero_counts(valid_mask)));
total_rows = n_valid; % Number of rows in the final matrix

% Preallocate arrays for sparse matrix construction - use efficient data types
...
element_offset = 0;
valid_idx_counter = 1; % New row index in the consolidated matrix
valid_indices = find(valid_mask);

% fprintf('Merging data: 0/%d', n_valid);

% --- Sequentially Merge Results from Parallel Loop ---
for i = 1:length(valid_indices)
    ...
    % Free up memory from processed cells
    all_rows{idx} = [];
    all_cols{idx} = [];
    all_vals{idx} = [];
    sortedjd_cell{idx} = [];
...
field_names = arrayfun(@(x) ['ray', num2str(x)], 1:n_valid, 'UniformOutput', false);
