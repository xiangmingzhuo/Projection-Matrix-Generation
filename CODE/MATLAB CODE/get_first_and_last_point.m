...
% 1. 计算射线在 ECEF 坐标系下的中点
mid_point_xyz = (points(1, :) + points(2, :)) / 2;
% 2. 将中点转换为 BLH (经纬度高)
[mid_lat, mid_lon, mid_h] = XYZtoBLH_sphere(mid_point_xyz(1), mid_point_xyz(2), mid_point_xyz(3), R);

% 转换坐标
[lat, lon, h] = XYZtoBLH_sphere(points(:,1), points(:,2), points(:,3),R);
BLH = [[lon, lat], h];  % [经度, 纬度, 高度]
...
% if ~(enter_point(1,2) >= lat_min && enter_point(1,2) <= lat_max && enter_point(1,1) >= lon_min && enter_point(1,1) <= lon_max)
%     is_valid = false;
%     return;
% end
% if ~(exit_point(1,2) >= lat_min && exit_point(1,2) <= lat_max && exit_point(1,1) >= lon_min && exit_point(1,1) <= lon_max)
%     is_valid = false;
%     return;
% end
% 检查高度条件 (提前返回)
...
% 计算包围盒边界
min_lon = min([enter_point(1), exit_point(1), mid_lon]);
max_lon = max([enter_point(1), exit_point(1), mid_lon]);
min_lat = min([enter_point(2), exit_point(2), mid_lat]);
max_lat = max([enter_point(2), exit_point(2), mid_lat]);
