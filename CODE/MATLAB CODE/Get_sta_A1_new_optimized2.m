...
function [one_dimensional_index,sortedjd_ddzb,values,station,satellite,scope_blh] = Get_sta_A1_new_optimized2(XD,YD,ZD,XS,YS,ZS,jdmin,jdmax,jdjg,wdmin,wdmax,wdjg,gdmin,gdmax,gdjg,jdmin_rad,wdmin_rad,jdmax_rad,wdmax_rad,jdjg_rad,wdjg_rad,total_length,nj,nw,ng,h_grid)
%%% 初始化输出
R = 6371000;
station = [XD,YD,ZD];
satellite = [XS,YS,ZS];
...
midpoint_blh(:,[1,2]) = rad2deg(midpoint_blh(:,[2,1]));
% 计算两两交点之间的截距值
...
%此函数采用二分法+球面三角法来对单独的射线生成A
function [one_dimensional_index,sortedjd_ddzb,values,station,satellite,scope_blh] = Get_sta_A1_optimized(XD,YD,ZD,XS,YS,ZS,jdmin,jdmax,jdjg,wdmin,wdmax,wdjg,gdmin,gdmax,gdjg,jdmin_rad,wdmin_rad,jdmax_rad,wdmax_rad,jdjg_rad,wdjg_rad,total_length,nj,nw,ng)
%%% 初始化输出
R = 6371000;
station = [XD,YD,ZD];
satellite = [XS,YS,ZS];
...
%% 稀疏矩阵优化
% total_length = round(((wdmax-wdmin)/wdjg) * ((jdmax-jdmin)/jdjg) * ((gdmax-gdmin)/gdjg));
one_dimensional_index = sparse(double(grid_indices), 1, values, total_length, 1);
