...
R = 6371000;
stations = unique(stations,'rows');
satellites = unique(satellites,'rows');
[stations(:,2), stations(:,1), stations(:,3)] = XYZtoBLH_sphere(stations(:,1), stations(:,2), stations(:,3),R);
stations = [rad2deg(stations(:,1)), rad2deg(stations(:,2)), stations(:,3)/1e+3];
[satellites(:,2), satellites(:,1), satellites(:,3)] = XYZtoBLH_sphere(satellites(:,1), satellites(:,2), satellites(:,3),R);
satellites = [rad2deg(satellites(:,1)), rad2deg(satellites(:,2)), satellites(:,3)/1e+3];
%% 网格参数
