...
% 最终确定首尾点的经纬度高度值
lon1 = max(approximateNumberDown(min_lon, jdjg) , lon_min);
lon2 = min(approximateNumberUp(max_lon, jdjg) , lon_max);
lat1 = max(approximateNumberDown(min_lat, wdjg) , lat_min);
lat2 = min(approximateNumberUp(max_lat, wdjg) , lat_max);
end
