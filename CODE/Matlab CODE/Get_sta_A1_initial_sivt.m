% This function uses equation solving to generate matrix A for a single ray.
function [one_dimensional_index,sortedjd_ddzb,values] =Get_sta_A1_initial_sivt(XD,YD,ZD,XS,YS,ZS,jd_params,wd_params,gd_params,total_length)
%%% Initialize output
...
% Pre-store grid boundary points
...
% latitude range and interval (in degrees), height range and interval, and the average
% Earth radius of the inversion area.
R=6371000;
...
for idx = 1:num_longitudes
    ...
    f6 = tan(current_J)*X - Y;  % Equation of the longitude plane
    % Solve the system of equations
...
for idx = 1:num_latitudes
    ...
    f5 = tan(current_W)^2*(X^2 + Y^2) - Z^2;  % Equation of the latitude plane
    % Solve the system of equations
...
sortedjd_ddzb([1, 2], :) = sortedjd_ddzb([2, 1], :);
