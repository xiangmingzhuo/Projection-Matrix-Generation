function [A, sortedjd, distance,station,satellite] = Get_A1_trangle_simple33(station, satellite, jdmin, jdmax, jdjg, wdmin, wdmax, wdjg, gdmin, gdmax, gdjg,jdmin_rad,wdmin_rad,jdmax_rad,wdmax_rad,jdjg_rad,wdjg_rad,total_length)
...
station_cell = cell(n_rays, 1);
satellite_cell = cell(n_rays, 1);
% --- Progress Tracking ---
...
for i = 1:n_rays
    ...
    try
        [A1, sorted_ddzb, distances] = Get_sta_A1_new_optimized(...
        ...
        if isempty(A1)
            ...
            station_cell{i} = station(i, :);
            satellite_cell{i} = satellite(i, :);
        end
    catch ME
...
all_station = cell(1, n_valid);
all_satellite = cell(1, n_valid);
...
for i = 1:length(valid_indices)
    ...
    all_station{valid_idx_counter} = station_cell{idx};
    all_satellite{valid_idx_counter} = satellite_cell{idx};
...
station = cell2struct(all_station, field_names, 2);
satellite = cell2struct(all_satellite, field_names, 2);
