function [A,L,sortedjd,distance]=Get_A1_optimized(doy,Sites_Info,sate,jdmin,jdmax,jdjg,wdmin,wdmax,wdjg,gdmin,gdmax,gdjg,t)
nj=(jdmax-jdmin)/jdjg;
nw=(wdmax-wdmin)/wdjg;
ng=(gdmax-gdmin)/gdjg;
nx=nj*nw*ng;
...
% station_cell = cell(n_rg2,1);
% satellite_cell = cell(n_rg2,1);
% 频率参数
...
%     station_local = cell(max_possible,1);
%     satellite_local = cell(max_possible,1);    
    valid_cnt = 0;
    ...
    for k=k0:k2
        for j=1:32
            ...
            % 球面三角计算
            [A1,sorted_ddzb,distances,station,satellite] = Get_sta_A1_optimized(sx,sy,sz,...
            ...
            if max(sorted_ddzb(3,:))<gdmax-100
                continue;
            end
...
%             station_local{valid_cnt} = station;
%             satellite_local{valid_cnt} = satellite;            
        end
    end
...
%     station_cell{i} = station_local(1:valid_cnt);
%     satellite_cell{i} = satellite_local(1:valid_cnt);    
end
toc
% 合并所有测站的结果
A = vertcat(A_cell{:});
...
% all_station = cell(1, total_elements);
% all_satellite = cell(1, total_elements);
% 填充数据
idx = 1;
...
%     all_station(idx:idx+current_length-1) = station_cell{i}(:);
%     all_satellite(idx:idx+current_length-1) = satellite_cell{i}(:);    
