function [A,L,sortedjd,distance]=Get_A1_canshu1(doy,Sites_Info,sate,jdmin,jdmax,jdjg,wdmin,wdmax,wdjg,gdmin,gdmax,gdjg,t)
nj=(jdmax-jdmin)/jdjg;
nw=(wdmax-wdmin)/wdjg;
ng=(gdmax-gdmin)/gdjg;
nx=nj*nw*ng;
...
parfor i=1:n_rg2
    ...
    for k=k0:k2
        for j=1:32
            ...
            % 球面三角计算
            [A1,sorted_ddzb,distances] = Get_sta_A1_canshu_optimized(sx,sy,sz,...
            ...
            % if max(sortedjd_ddzb(3,:))<gdmax-gdjg
            %     continue;
            % end
...
% % 将 cell 数组转换为列向量
% sortedjd_cell_flat = vertcat(sortedjd_cell{:});
% distance_cell_flat = vertcat(distance_cell{:});
% 
% % 生成字段名
% field_names = arrayfun(@(x) ['shexian' num2str(x)], 1:numel(sortedjd_cell_flat), 'UniformOutput', false);
% 
% % 使用 cell2struct 一次性创建结构体
% sortedjd = cell2struct(sortedjd_cell_flat, field_names, 2);
% distance = cell2struct(distance_cell_flat, field_names, 2);
end